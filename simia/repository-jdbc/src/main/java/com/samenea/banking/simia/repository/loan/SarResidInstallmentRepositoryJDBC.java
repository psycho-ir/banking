...
import com.samenea.banking.simia.model.repository.SarResidInstallmentRepository;
...
@Repository
public class SarResidInstallmentRepositoryJDBC extends BasicRepositoryJDBC implements SarResidInstallmentRepository {

    private static final String FIND_CREDIT_ACCOUNT_NUMBER_QUERY = "select f50 from dbx4_tx42 where f3 = (select f3 from dbx4_tx423 where fvam = ?)";
    private static final String FIND_DEBIT_KARMOZD_QUERY = "select f52 from dbx4_tx42 where f3 = (select f3 from dbx4_tx423 where fvam = ?)";
